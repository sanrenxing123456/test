...
class VCocoDetection(Dataset):
    def __init__(self,
                 ...
                 transforms=None,model_vit=None):
        self.vitposemodel=model_vit
        ...
        # COCO initialize
        self.coco = COCO(self.all_file)#jiazai coco shuju,peichengzidian
    ...
    def load_pair_annotations(self, image_index):
        ...
        for idx in range(num_ann):
            ...
            for o_id in o_act.keys():
                ...
                if o_id == -1: ## obj is empty, set obj as self
    ...
    # Check Method
    ############################################################################
    def sum_action_ann_for_id(self, find_idx):#zhaodaomeigezhenshizuoledongzuodezhonglei
        sum = 0
    ...
    def prepare_img(self, idx):
        ...
        # target["orig_img"]=torch.as_tensor(np.array(image))#h,w,3
        # target["orig_img"]=None#h,w,3
    ...
    def get_ann_info(self, idx):
        ...
        sample = {
            ...
            'pair_boxes': torch.as_tensor(pair_bbox, dtype=torch.float32), ## tail box might be all -1 (overlapped or verb with no obj, like run etc)
...
def make_hoi_transforms(image_set):#根据不同的图像集（训练集、验证集、测试集）创建相应的图像变换流水线
    normalize = T.Compose([
        T.ToTensor(),
        ...
        return T.Compose([
            ...
            T.RandomSelect(
                ...
                T.Compose([
                    ...
                    # T.RandomSizeCrop(384, 600), # may remove boxes
                    T.RandomResize(scales, max_size=1333),
                ])
            ),
            normalize,
...
def build(image_set, args,model_vit):
