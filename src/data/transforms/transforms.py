...
def resize(image, target, size, max_size=None):
    ...
    size = get_size(image.size, size, max_size)#ruguo size shi yigebiaoliang,zechuliwanhoufanhuichicun,ruguosizeshiyige list or tuple,zefanzhuanhouzhijiefanhuis
    rescaled_image = F.resize(image, size)

    if target is None:
        return rescaled_image, None

    ratios = tuple(float(s) / float(s_orig) for s, s_orig in zip(rescaled_image.size, image.size))#宽度和高度的缩放比例
