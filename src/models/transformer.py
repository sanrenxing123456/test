...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        if graph_embedding !=None:
            tgt2 = self.multihead_attn(query=tgt,
                                   key=graph_embedding,
                                   value=graph_embedding, attn_mask=None,
                                   key_padding_mask=None,
                                   memory_role_embedding=None)[0]
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=tgt2,
                                   key=graph_embedding,
                                   value=graph_embedding, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask,
                                   memory_role_embedding=None)[0]
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),
                                   ...
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask,
                                   memory_role_embedding=graph_embedding)[0]
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
    ...
    def forward(self, tgt, memory,
        ...
        if self.normalize_before:
            return self.forward_pre(tgt, memory, tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos, memory_role_embedding, query_structure_encoding,graph_embedding)
